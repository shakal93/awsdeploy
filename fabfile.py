...
# 원격 PC 에서 자동으로 서버를 세팅하는 자동화 지원모듈이다
...
def deploy():
    ...
    # 가상환경 업데이트
    _update_virtualenv()
    # 서버가 가동될때 홈디렉토리, 로그파일의 위치등등 설정
    # *.conf가 수정 및 생성된다
    # wsgi.py 엔트리포인트가 설정되어 있다
    _make_virtualhost()
    # 권한 조정 => ubuntu가 접속할 수 있게 조정
    _grant_apache2()
    # 아파치 서버 재가동
    _restart_apache2()
...
def _update_virtualenv():
    ...
    # 해당 가상환경 안에, flask, scikit-learn 이 2개가 설치될 것이다
